...
inline int ssd1306_get_font(uint8_t character) {
  ...
  if (character > 126) return 0;

  return (character - 32) * FONT_WIDTH;
}
...
void ssd1306_draw_char(uint8_t *ssd, int16_t x, int16_t y, uint8_t character) {
    ...
    int page = y / 8;
    int bit_offset = y % 8;  // offset within the page
    int fb_idx = page * 128 + x;

    for (int i = 0; i < FONT_WIDTH; i++) {
        uint8_t font_data = font[idx + i];

        ssd[fb_idx++] |= font_data << bit_offset;

        // if bits overflow into next page
        if (bit_offset > 0 && page < (ssd1306_height / 8) - 1) {
            ssd[fb_idx + 128 - 1] |= font_data >> (8 - bit_offset);
        }
    }
}
...
void ssd1306_print_cursor(uint8_t *ssd, uint8_t x, uint8_t y) {
  char pixel_map[CURSOR_HEIGHT][CURSOR_HEIGHT]= {
    {0, 2, 2, 2, 2, 2, 2, 2, 2, 2},
    {0, 0, 2, 2, 2, 2, 2, 2, 2, 2},
    {0, 1, 0, 2, 2, 2, 2, 2, 2, 2},
    {0, 1, 1, 0, 2, 2, 2, 2, 2, 2},
  ...
  for (int i=0; i<CURSOR_WIDTH;i++) {
    for (int j=0; j<CURSOR_HEIGHT;j++) {
      if (pixel_map[j][i] == 2) continue;
      if (x+i >= ssd1306_width) continue;
      if (y+j >= ssd1306_height) continue;
      ssd1306_set_pixel(ssd, x+i, y+j, pixel_map[j][i]);
    }
  }
}
